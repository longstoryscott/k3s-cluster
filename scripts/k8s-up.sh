...
for node in "${NODES[@]}"; do
    ssh "${node}.local" "mkdir -p ~/.kube && touch ~/.kube/config && curl -sfL https://get.k3s.io | K3S_URL=https://${MASTER_IP}:6443 K3S_NODE_NAME=\"${node}\" K3S_TOKEN=${TOKEN} K3S_KUBECONFIG_MODE='600' sh -s - && echo \"${CONF}\" > ~/.kube/config && sudo chmod 600 ~/.kube/config"
done
